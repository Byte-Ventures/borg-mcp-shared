...
function exactKeys(value, allowed, required) {
    for (const key of Object.keys(value)) {
        if (!allowed.includes(key))
            throw new ProtocolContractError(`Unknown SSE data field "${key}".`);
    }
...
export function decodeEnrichedStreamEntry(value) {
    ...
    return {
        ...
        recipient_drone_ids: entry.recipient_drone_ids.map((id, index) => decodeUuid(id, ['entry', 'recipient_drone_ids', index])),
    };
}
...
export function decodeSseFrames(input) {
...
function decodeFrame(frame) {
    ...
    for (const line of frame.split('\n')) {
        ...
        if (field === 'event') {
            if (eventSeen)
                throw new ProtocolContractError('Duplicate SSE event field.');
            eventSeen = true;
            eventName = decodeOpaqueIdentifier(value, ['event']);
        }
        else if (field === 'id') {
            if (idSeen)
                throw new ProtocolContractError('Duplicate SSE id field.');
            idSeen = true;
            ...
            throw new ProtocolContractError(`Unsupported SSE field "${field}".`);
    }
    ...
    if (eventName === 'log') {
        if (!id)
            throw new ProtocolContractError('Log SSE events require an id field.');
        exactKeys(data, ['cursor', 'entry'], ['cursor', 'entry']);
        const cursor = decodeLogCursor(data.cursor, ['cursor']);
        if (cursor.id !== id)
            throw new ProtocolContractError('SSE id and cursor id differ.');
    ...
    exactKeys(data, ['as_of', 'replay_complete', 'next_cursor', 'cursor_status'], ['as_of', 'replay_complete']);
    ...
    if (data.next_cursor !== undefined)
        bookmark.next_cursor = decodeLogCursor(data.next_cursor);
    if (data.cursor_status !== undefined) {
        if (!['valid', 'expired', 'unknown'].includes(String(data.cursor_status))) {
            throw new ProtocolContractError('Invalid bookmark cursor_status.');
        }
        bookmark.cursor_status = data.cursor_status;
    }
    return bookmark;
}
