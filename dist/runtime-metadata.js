...
export function validateWorkingRepoName(value) {
...
function originParts(value) {
    if (byteLength(value) > RUNTIME_METADATA_LIMITS.repo_origin_bytes ||
        value.includes('%') ||
    ...
    if (scp)
        return { host: validatePublicHost(scp[1]), path: scp[2] };
...
export function canonicalizeRepositoryIdentity(inputOrigin, expectedName) {
...
export function validateRuntimeMetadata(value) {
    ...
    if (metadata.working_repo_name !== null && metadata.working_repo_origin !== null) {
        const canonical = canonicalizeRepositoryIdentity(metadata.working_repo_origin, metadata.working_repo_name);
...
export function validateRuntimeMetadataPatch(value) {
    ...
    if (hasName && hasOrigin) {
        ...
        if (name !== null && name !== undefined && origin !== null && origin !== undefined) {
            const canonical = canonicalizeRepositoryIdentity(origin, name);
