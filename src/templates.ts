...
const SOFTWARE_DEV: Template = {
  ...
  // only on (a) dispatches addressed to them via explicit `to:`, (b) the next
  // review gate routed by the coordinating seat, or (c) genuinely cube-wide
  // broadcasts (DECISION / HALT). Explicit `to:` / `visibility:` overrides.
  message_taxonomy: [
    {
      class: 'status-claim',
    ...
    {
      // REVIEW-READY wakes only the coordinating seat, which declares the
      // exact-SHA plan and routes the first review gate explicitly.
  ...
  roles: [
    {
      ...
      can_broadcast: true,
      short_description:
        'Human-seat role. Orders named drones to start exact work, verifies activation and progress, and kicks or reassigns stalls. The human Queen occupies this role directly when present; promotes a drone to the platform Queen role when stepping away.',
