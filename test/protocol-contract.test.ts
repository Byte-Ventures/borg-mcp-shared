...
import * as sharedApi from '../src/index.js';
  ...
  const tagPreflight = { protocol_version: '14' } as const;
...
describe('package and handshake contract', () => {
  it('keeps the exported identity aligned with package.json', async () => {
    ...
    expect(SHARED_PACKAGE_VERSION).toBe('2.0.0');
  ...
  it('uses a bodyless health path and a credential-free protocol preflight', () => {
    ...
    expect(DOCUMENTS_PATH).toBe('/api/cubes/:cubeId/documents');
    expect(DOCUMENT_PATH).toBe('/api/cubes/:cubeId/documents/:documentId');
    ...
    expect(PROTOCOL_HTTP_CONTRACT).toMatchObject({
      ...
      document_put: { method: 'PUT', success_status: 201, authenticated: true, mutation: true },
      document_list: { method: 'GET', success_status: 200, authenticated: true, mutation: false },
      document_get: { method: 'GET', success_status: 200, authenticated: true, mutation: false },
      document_remove: { method: 'DELETE', success_status: 200, authenticated: true, mutation: true },
      cube_delete: {
        method: 'DELETE',
  ...
  it('emits and decodes a tag-only preflight carrying nothing but the exact tag', () => {
    ...
    expect(emitted).toEqual({ protocol_version: '14' });
  ...
  it('never reflects an untrusted protocol_version at any wrapper boundary', () => {
    ...
    const boundaries: Array<[string, (protocol_version: unknown) => unknown, string]> = [
      ...
      ['cube delete request envelope', (v) => decodeDeleteCubeRequestEnvelope({ protocol_version: v, request_id: req, payload: secretPayload }), 'Unsupported protocol version.'],
      ['cube delete response envelope', (v) => decodeDeleteCubeResponseEnvelope({ protocol_version: v, request_id: req, payload: secretPayload }), 'Unsupported protocol version.'],
  ...
  it('rejects removed unrepresented error codes', () => {
    for (const code of [
  ...
  it('keeps deleted, revoked, and evicted outcomes distinct', () => {
...
describe('cube-deletion codecs', () => {
  const cubeId = '10000000-0000-4000-8000-000000000001';

  it('strictly decodes empty requests and terminal success envelopes', () => {
    expect(decodeDeleteCubeRequest({})).toEqual({});
    expect(decodeDeleteCubeRequestEnvelope(createProtocolEnvelope('delete-cube-1', {})).payload)
      .toEqual({});
    expect(decodeDeleteCubeResponse({ cube_id: cubeId, deleted: true })).toEqual({
      cube_id: cubeId,
      deleted: true,
    });
    expect(decodeDeleteCubeResponseEnvelope(createProtocolEnvelope('delete-cube-1', {
      cube_id: cubeId,
      deleted: true,
    })).payload).toEqual({ cube_id: cubeId, deleted: true });
  });

  it('rejects fields in requests and malformed terminal responses', () => {
    expect(() => decodeDeleteCubeRequest({ confirm: true })).toThrow(ProtocolContractError);
    expect(() => decodeDeleteCubeResponse({ cube_id: cubeId, deleted: false }))
      .toThrow(ProtocolContractError);
    expect(() => decodeDeleteCubeResponse({ cube_id: 'not-a-uuid', deleted: true }))
      .toThrow(ProtocolContractError);
    expect(() => decodeDeleteCubeResponse({ cube_id: cubeId, deleted: true, cascade: true }))
      .toThrow(ProtocolContractError);
  });
});
...
describe('cube creation codecs', () => {
  ...
  it.each(['software-dev', 'starter', 'local-model'] as const)(
  ...
  it('rejects malformed identities, tuple fields, templates, and extra authority', () => {
    ...
    expect(() => decodeCreateCubeRequest({ ...request, template: 'default' })).toThrow(ProtocolContractError);
...
describe('coordination request codecs', () => {
  ...
  it('decodes and validates the optional append-log routing response', () => {
    ...
    expect(decodeAppendLogResultEnvelope({
      ...
      payload,
    })).toEqual({ protocol_version: '14', request_id: 'append-log-1', payload });
  });
  ...
  it('decodes acknowledgement status without conflating missing acks and claims', () => {
    const entryId = '00000000-0000-4000-8000-000000000001';
    const recipientId = '00000000-0000-4000-8000-000000000002';
    const claimantId = '00000000-0000-4000-8000-000000000003';
    const request = { entry_id: entryId };
    const result = {
      entry_id: entryId,
      visibility: 'direct' as const,
      recipients: [{
        drone_id: recipientId,
        drone_label: 'builder-00000000',
        drone_role: 'Builder',
        ...
        drone_label: null,
        drone_role: null,
        acknowledged_at: '2026-08-16T13:59:00.000Z',
      }],
      claims: [{
        drone_id: claimantId,
        drone_label: null,
        drone_role: null,
        claimed_at: '2026-08-16T14:00:00.000Z',
      }],
    };

    expect(decodeAckStatusRequest(request)).toEqual(request);
    expect(decodeAckStatusRequestEnvelope(createProtocolEnvelope('ack-status-1', request)).payload)
      .toEqual(request);
    expect(decodeAckStatusResult(result)).toEqual(result);
    expect(decodeAckStatusResultEnvelope(createProtocolEnvelope('ack-status-1', result)).payload)
      .toEqual(result);
    expect(decodeAckStatusResult(result).recipients[0]?.acknowledged_at).toBeNull();
    expect(decodeAckStatusResult(result).claims).toHaveLength(1);
  });

  it('decodes full and short entry queries without cursor state', () => {
    const fullId = '10000000-0000-4000-8000-000000000001';
    const entry = {
      id: fullId,
      cube_id: '20000000-0000-4000-8000-000000000001',
      drone_id: null,
      ...
      drone_label: null,
      role_name: null,
      recipient_drone_ids: ['30000000-0000-4000-8000-000000000001'],
    };
    expect(decodeEntryQueryRequest({ entry_id: fullId.toUpperCase() })).toEqual({ entry_id: fullId });
    expect(decodeEntryQueryRequest({ entry_id: 'ABCDEF12' })).toEqual({ entry_id: 'abcdef12' });
    expect(decodeEntryQueryRequestEnvelope(
      createProtocolEnvelope('entry-query-1', { entry_id: 'abcdef12' }),
    ).payload).toEqual({ entry_id: 'abcdef12' });
    expect(decodeEntryQueryResult({ entry })).toEqual({ entry });
    expect(decodeEntryQueryResultEnvelope(
      createProtocolEnvelope('entry-query-1', { entry }),
    ).payload).toEqual({ entry });
  });

  it('rejects malformed or ambiguous entry-query wire shapes', () => {
    const entry_id = '10000000-0000-4000-8000-000000000001';
    for (const request of [
      {},
      { entry_id: 'abcdef1' },
      { entry_id: 'abcdefghi' },
      { entry_id, cursor: null },
    ]) {
      expect(() => decodeEntryQueryRequest(request)).toThrow(ProtocolContractError);
    }
    expect(() => decodeEntryQueryResult({ entry: appendLogEntry, cursor: null }))
      .toThrow(ProtocolContractError);
  });

  it('rejects malformed acknowledgement-status contracts', () => {
    const entryId = '00000000-0000-4000-8000-000000000001';
    const recipient = {
      drone_id: '00000000-0000-4000-8000-000000000002',
      drone_label: 'builder-00000000',
      drone_role: 'Builder',
      acknowledged_at: null,
    };
    ...
    expect(() => decodeAckStatusRequest({ entry_id: entryId, cursor: null }))
      .toThrow(ProtocolContractError);
    expect(() => decodeAckStatusResult({ ...result, visibility: 'private' }))
      .toThrow(ProtocolContractError);
    expect(() => decodeAckStatusResult({
      ...result,
      recipients: [{ ...recipient, acknowledged_at: 'yesterday' }],
    })).toThrow(ProtocolContractError);
    expect(() => decodeAckStatusResult({
      ...result,
      claims: [{
        drone_id: recipient.drone_id,
        drone_label: recipient.drone_label,
        drone_role: recipient.drone_role,
        claimed_at: '2026-08-16T14:00:00.000Z',
        stale: false,
      }],
    })).toThrow(ProtocolContractError);
  ...
  it('decodes read-log results without the retired claims field', () => {
    const result = {
      entries: [],
      cursor: null,
      behind_by: 0,
    ...
    expect(decodeReadLogResult(result)).toEqual(result);
    expect(() => decodeReadLogResult({ ...result, claims: [] })).toThrow(
      'Unknown coordination field "claims".',
    );
  });
...
describe('clean-slate attach wire types', () => {
  ...
  it('decodes attach response envelope with correct protocol version', () => {
    ...
    expect(decoded.protocol_version).toBe('14');
  ...
  it('creates and decodes a valid attach request envelope round-trip', () => {
    ...
    expect(envelope.protocol_version).toBe('14');
