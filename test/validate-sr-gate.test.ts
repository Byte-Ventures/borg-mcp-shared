...
describe('SR artifact-tuple gate', () => {
  ...
  describe('validateSourceRun', () => {
    ...
    it('accepts a valid source run', () => {
      expect(validateSourceRun(validRun, '.github/workflows/publish.yml', 'push', 'v0.4.0', 'abc123', 1)).toBeNull();
    });

    it.each([
      [{ ...validRun, path: '.github/workflows/ci.yml' }, 'workflow path'],
      [{ ...validRun, event: 'workflow_dispatch' }, 'event'],
      [{ ...validRun, head_branch: 'refs/tags/v0.4.0' }, 'head_branch'],
      ...
      const error = validateSourceRun(run, '.github/workflows/publish.yml', 'push', 'v0.4.0', 'abc123', 1);
  ...
  describe('validateArtifactContents', () => {
    it('accepts valid artifact contents', () => {
      const files = [
        'borgmcp-shared-0.4.0.tgz',
        'SHA512SUMS',
        'RUN_EVIDENCE',
        'artifact-report.json',
        'borgmcp-shared-0.4.0.cdx.json',
        'sbom-report.json',
      ];
      expect(validateArtifactContents(files, 'borgmcp-shared-0.4.0.tgz')).toBeNull();
    });

    it('rejects missing tarball', () => {
      expect(validateArtifactContents(['SHA512SUMS'], 'borgmcp-shared-0.4.0.tgz')).toContain('tarball');
    });
    ...
    it('rejects unexpected entries', () => {
      ...
      const error = validateArtifactContents(files, 'borgmcp-shared-0.4.0.tgz');
